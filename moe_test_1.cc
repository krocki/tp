  ...
  for (int kk = 0; kk < top_k; ++kk) {
    ...
    for (int lp = 0; lp < local_p; ++lp) {
      threads.emplace_back([&, lp, expert_id]() {
        ...
        serial_matmul(x, Wg_shards[lp * n_experts + expert_id].data, lg, 1, local_dff, d_model);

        // Up proj (column-parallel)
        serial_matmul(x, Wu_shards[lp * n_experts + expert_id].data, lu, 1, local_dff, d_model);
    ...
    // Add weighted
    for (int i = 0; i < d_model; ++i) {
      out[i] += expert_weight * global_sum[i];
    }
...
void save_full_weights(const QwenConfig* cfg, float* full_router, float** full_Wg, float** full_Wu, float** full_Wd) {
  ...
  // Save router
  std::ofstream out_router("router.bin", std::ios::binary);
  out_router.write((char*)full_router, n_experts * d_model * sizeof(float));
  out_router.close();

  for (int e = 0; e < n_experts; ++e) {
    std::string base = "expert" + std::to_string(e) + "_";
...
int main(int argc, char** argv) {
  ...
  if (mpi_rank == 0) {
    ...
    delete[] full_Wu;
    delete[] full_Wd;
    delete[] out_serial;  // Not used further
  }
  ...
  std::ifstream in_router("router.bin", std::ios::binary);
  ...
  Profiler prof_tp;
  moe_layer_tp(out_tp, x, Wg_shards, Wu_shards, Wd_shards, router_w, &cfg, 1, local_p, mpi_rank, mpi_size, prof_tp);
  if (mpi_rank == 0) {
    prof_tp.print("TP");
